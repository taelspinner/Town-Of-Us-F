            ...
            if (!UntransportablePlayers.ContainsKey(TransportPlayer1.PlayerId) && !UntransportablePlayers.ContainsKey(TransportPlayer2.PlayerId))
            {
                ...
                if (TransportPlayer1.Is(RoleEnum.Pestilence) || TransportPlayer1.IsOnAlert() || TransportPlayer2.IsBodyguarded())
                {
                    ...
                    else if (!Player.IsProtected())
                    {
                        ...
                        Coroutines.Start(TransportPlayers(TransportPlayer1.PlayerId, Player.PlayerId, !blockVetAlert));

                        Utils.Rpc(CustomRPC.Transport, TransportPlayer1.PlayerId, Player.PlayerId, !blockVetAlert);
                        return;
                    }
                    transRole.LastTransported = DateTime.UtcNow;
                    return;
